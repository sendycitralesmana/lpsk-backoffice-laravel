<div class="modal fade" id="edit-{{ $setting->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form role="form" method="POST" action="/backoffice/setting/{{ $setting->id }}/update" enctype="multipart/form-data">
                {{ csrf_field() }}
                @method('PUT')
                    ...
                    <div class="card card-outline card-primary">
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select name="setting_category_id" class="form-control select2 @if($errors->has('setting_category_id')) is-invalid @endif" 
                                    ...
                                    @foreach ($settingCategories as $settingCategory)
                                        <option value="{{ $settingCategory->id }}" {{ $setting->setting_category_id == $settingCategory->id ? 'selected' : '' }}>{{ $settingCategory->name }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('setting_category_id'))
                                <small class="help-block" style="color: red">{{ $errors->first('setting_category_id') }}</small>
                                @endif
                            </div>
                            <div class="form-group">
                                <label for="foto">Sampul</label>
                                @if ($setting->cover)
                                    <img src="{{ Storage::disk('s3')->url($setting->cover) }}" class="reviewCover rounded img-fluid mb-3 col-sm-5 d-block"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text"  name="title" class="form-control @if($errors->has('title')) is-invalid @endif" value="{{ $setting->title }}"
                            ...
                            <div class="form-group">
                                ...
                                required oninput="this.setCustomValidity('')" oninvalid="this.setCustomValidity('Konten harus diisi')">{{ $setting->content }}</textarea>
                                @if($errors->has('url'))
                            ...
                            @if (auth()->user()->id == 1)
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control @if($errors->has('status')) is-invalid @endif" 
                                        ...
                                        <option value="DIAJUKAN" {{ $setting->status == 'DIAJUKAN' ? 'selected' : '' }}>DIAJUKAN</option>
                                        <option value="DINAIKAN" {{ $setting->status == 'DINAIKAN' ? 'selected' : '' }}>DINAIKAN</option>
                                        <option value="DITURUNKAN" {{ $setting->status == 'DITURUNKAN' ? 'selected' : '' }}>DITURUNKAN</option>
                                    </select>
